...
class SaleOrderController extends Controller
{
    ...
    { 
        ...
        $data = $data->with(['items.product','customer']);
        return response()->json([
            'query' => '',
            "message" => "Get Records Successfully",
    ...
    public function show($id)
    {
        ...
        $module = SaleOrder::where('id',$id)->with(['customer','items.product.unit','items.product.category'])->first();
    ...
    {
        if($id == 0){
               $module = New SaleOrder();  
        }else{
            $module = SaleOrder::where('id',$id)->first();
            if(!$module){
        ...
        $validations = [
            ...
            'serial_no' => ['nullable','string','unique:sale_orders,serial_no'],
            'ref' => ['nullable','string'],
            'date' => ['required'],
            'tax' => ['nullable'],
            'discount' => ['nullable'],
        ...
        if($id != 0){
            $validations['serial_no'] = ['nullable','string','unique:sale_orders,serial_no,'.$id];
        }
    ...
    {
        $module = SaleOrder::find($id);
        if($module == null){
             return response()->json(["message" => 'Record Deleted Successfully'],200);
        }
        
        $module->delete();
        SaleOrderItem::where('parent_id',$id)->delete();
        return response()->json([
