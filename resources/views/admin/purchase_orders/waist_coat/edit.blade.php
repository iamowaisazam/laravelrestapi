...
<div class="container-fluid">
    ...
    <div class="row">
      <div class="col-12">
        <form method="post" action="{{route('admin.purchase_orders.waist_coat_update',$orderid)}}"  >
            @csrf
            ...
            <div class="container-fluid">
                    <div class="table-responsive">
                        <table>
                            <thead>
                                <tr>
                                    <th style="text-align: center" >Waist Coat</th>
                                    <th style="text-align: center" >Remarks</th>
                                    <th style="text-align: center">Color</th>
                                    <th style="text-align: center">Size</th>
                                    <th style="text-align: center">Fabric</th>
                                    <th style="text-align: center">Description</th>
                                    <th style="text-align: center">Furing</th>
                                    <th style="text-align: center">Button</th>
                                    <th style="text-align: center">Label</th>
                                    <th style="text-align: center">Lining</th>
                            ...
                            <tbody class="items-list">  
                                ...
                                <tr>
                                    <td style="width: 120px;" >{{$item->attribute->title}}
                                    ...
                                    <td style="width: 140px;">
                                        <select name="items[{{$key}}][fabric]"  class="form-control" >
                                            @foreach ($fabrics as $fabric)
                                            <option @if($fabric->title == $item->fabric){{'selected'}} @endif >{{$fabric->title}}</option>    
                                            @endforeach
                                        </select>
                                    </td>
                                    ...
                                    <td style="width: 120px;">
                                        <select name="items[{{$key}}][furing]"  class="form-control" >
                                            @foreach ($furings as $furing)
                                            <option @if($furing->title == $item->furing){{'selected'}} @endif >{{$furing->title}}</option>    
                                            @endforeach
                                        </select>
                                    </td>
                                    <td style="width: 120px;">
                                        <select name="items[{{$key}}][button]"  class="form-control" >
                                            @foreach ($buttons as $button)
                                            <option @if($button->title == $item->button){{'selected'}} @endif >{{$button->title}}</option>    
                                            @endforeach
                                        </select>
                                    </td>
                                    <td style="width: 120px;">
                                        <select name="items[{{$key}}][label]"  class="form-control" >
                                            @foreach ($labels as $label)
                                            <option @if($label->title == $item->label){{'selected'}} @endif >{{$label->title}}</option>    
                                            @endforeach
                                        </select>
                                    </td>
                                    <td style="width: 120px;">
                                        <select name="items[{{$key}}][lining]"  class="form-control" >
                                            @foreach ($linings as $lining)
                                            <option @if($lining->title == $item->lining){{'selected'}} @endif >{{$lining->title}}</option>    
                                            @endforeach
                                        </select>
                                    </td>
                                    <td style="width: 82px;">
                                    ...
                                    <td style="width: 100px;" class="text-center total">0</td>
        ...
        $('.addproduct').click(() => {
            ...
            if(productId != null){
                $('.items-list').append(` <tr>
                                    <td style="width: 120px;" >${productTitle}
                                        <input value="${productId}" name="items[${key}][attribute_id]" type="hidden" />
                                    </td>
                                    <td style="width: 100px;">
                                        <input name="items[${key}][remarks]" class="form-control"/>
                                    </td>
                                    <td style="width: 140px;">
                                        <select name="items[${key}][color]" class="form-control" >
                                            @foreach($colors as $color)
                                    ...
                                    <td style="width: 100px;" class="text-center total">0</td>
                                    <td class="text-center"><button type="button" class="delete_item btn-little"><i class="text-danger fa-2x fas fa-window-close"></i></button></td></tr>`);
                calculate();
            }
        });
